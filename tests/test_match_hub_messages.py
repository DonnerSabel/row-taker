...
def test_match_hub_drives_trick_via_messages() -> None:
    ...
    hub.handle_client_message(SubmitCard(PlayCardCommand(PlayerID('player-0'), 1)))
    hub.handle_client_message(SubmitCard(PlayCardCommand(PlayerID('player-1'), 90)))
    ...
    hub.handle_client_message(SubmitRowChoice(ChooseRowCommand(PlayerID('player-0'), RowID('row-1'))))
    messages = hub.drain_outbox()
    ...
    assert [delta.played_card.value for delta in trick.deltas] == [1, 90]
