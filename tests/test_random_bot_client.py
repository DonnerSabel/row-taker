    ...
    return PlayerState(
        public_state=PublicState(
            ...
            players=[
                ...
                PublicPlayerInfo(
                    player_id=PlayerID("player-1"), name="Alice", score=0, hand_count=1
                ),
            ],
            rows=[
                Row(row_id=RowID("row-0"), cards=[Card(10)]),
            ...
            phase_info=PhaseInfo(
                ...
                active_player_id=PlayerID("player-0"),
                selectable_row_ids=(RowID("row-1"), RowID("row-3")),
            ),
        ),
        self_player_id=PlayerID("player-0"),
        hand=[Card(5)],
...
def test_random_bot_client_uses_game_client_core_for_card_choice() -> None:
    ...
    responses = bot.handle_server_message(
        ChooseCardRequested(player_id=PlayerID("player-0"), state=_choose_card_state())
    )
...
def test_random_bot_client_uses_game_client_core_for_row_choice() -> None:
    bot = RandomBotClient(rng=random.Random(1234))
    responses = bot.handle_server_message(
        ChooseRowRequested(player_id=PlayerID("player-0"), state=_choose_row_state())
    )
    assert len(responses) == 1
