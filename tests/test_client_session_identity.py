...
def _lobby() -> LobbyView:
    return LobbyView(
        seat_count=3,
        participants=(
            LobbyParticipantView(client_id="client-1", display_name="Alice", participant_kind="human", seat_index=None),
            LobbyParticipantView(client_id="client-2", display_name="Bob", participant_kind="human", seat_index=1),
            LobbyParticipantView(client_id="pending-bot-seat-2", display_name="Bot_1", participant_kind="bot", seat_index=2),
        ),
        seats=(
            LobbySeatView(seat_index=0, occupant_client_id=None, occupant_display_name=None, occupant_kind=None),
            LobbySeatView(seat_index=1, occupant_client_id="client-2", occupant_display_name="Bob", occupant_kind="human"),
            LobbySeatView(seat_index=2, occupant_client_id="pending-bot-seat-2", occupant_display_name="Bot_1", occupant_kind="bot"),
        ),
        game_started=False,
    )
...
def test_client_session_stores_own_client_id_from_identity_assigned() -> None:
    session = ClientSession(transport=_FakeTransport(), ui_client=_FakeUiClient(), interactive=False)
...
def test_assign_seat_uses_explicit_own_client_id() -> None:
    ...
    session = ClientSession(transport=transport, ui_client=_FakeUiClient(), interactive=False, own_client_id="client-1")

    mode = session._handle_lobby_command(_lobby(), ("seat", 0), "m")

    assert mode == ("main", None)
    assert transport.sent_messages == [AssignSeatToClient(seat_index=0, target_client_id="client-1")]
...
def test_render_lobby_shows_participants_and_marks_own_client(capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("row_taker.cli.client_session.clear_screen", lambda: None)
    session = ClientSession(transport=_FakeTransport(), ui_client=_FakeUiClient(), interactive=False, own_client_id="client-1")
