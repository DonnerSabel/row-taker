...
def test_overflow_uses_before_and_after_snapshots_with_one_motion() -> None:
    ...
    assert before_row.card_values == tuple(card.card_value for card in before_row.taken_cards)
...
@pytest.mark.parametrize(
    ("event", "message"),
    [
        (PresentationTrickFinished(), None),
        (PresentationRoundFinished(), "Runde beendet"),
        (PresentationGameFinished(), "Spiel beendet"),
    ],
)
def test_finish_events_are_complete_visual_states(event, message: str | None) -> None:
    ...
    assert visual_state.status.message_line == message
