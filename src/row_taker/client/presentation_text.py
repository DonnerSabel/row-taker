...
def format_presentation_event(event: PresentationEvent) -> str:
    ...
    if isinstance(event, PresentationCardsRevealed):
        cards = ", ".join(
            f"{play.player_name}:{play.card_value}" for play in event.plays
        )
    ...
    if isinstance(event, PresentationRowTaken):
        return (
            f"Reihe genommen -> {event.player_name} erhielt "
            f"{event.bullheads} Hornochsen"
        )
    if isinstance(event, PresentationOverflowResolved):
        return (
            f"Überlauf aufgelöst -> {event.player_name} erhielt "
            f"{event.bullheads} Hornochsen"
        )
